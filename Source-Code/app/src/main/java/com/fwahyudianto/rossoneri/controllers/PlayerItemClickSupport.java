...
public class PlayerItemClickSupport {
    ...
    //  Constructor
    private PlayerItemClickSupport(RecyclerView m_recyclerView) {
        m_RecyclerView = m_recyclerView;
        m_RecyclerView.setTag(R.id.item_click_support, this);
        m_RecyclerView.addOnChildAttachStateChangeListener(mAttachListener);
    }

    //  Getter
    public static PlayerItemClickSupport addTo(RecyclerView p_oRecyclerView) {
        PlayerItemClickSupport oSupport = (PlayerItemClickSupport) p_oRecyclerView.getTag(R.id.item_click_support);
        if (oSupport == null) {
            oSupport = new PlayerItemClickSupport(p_oRecyclerView);
        }

        return oSupport;
    }

    public static PlayerItemClickSupport removeFrom(RecyclerView p_oRecyclerView) {
        PlayerItemClickSupport oSupport = (PlayerItemClickSupport) p_oRecyclerView.getTag(R.id.item_click_support);
        if (oSupport != null) {
